<%@ Page Language="C#" MasterPageFile="MP.Master" AutoEventWireup="true" Inherits="WebSite.ProductPage2" %>
<asp:Content ID="Content2" ContentPlaceHolderID="cp" runat="server">
		<div class="row">
			...
			<template v-if="productData.CanBeAddedToCart && !productData.IsCombine">
					<span id="divQty" v-if="productData.Code != 'CertificatCadeau'">Quantity:
						...
						<template v-if="filter.Values.length <= matrixMaxElementBeforeDropDown">
							...
							<div class="matrix-filter">
								<div v-if="filter.Values.length > 1"  @click="setFilterSelected(filter, null)" class="filter-choice choice-all"
									:class="{'selected': !filter.selected}" id="matrix-filter-view-all"> View all </div>
							</div>
						</template>
					</div>
				</div>
				<div id="matrix-pictures" v-if="matrixProductsMatchingFilter && matrixProductsMatchingFilter.length > 0">
					<div class="header row">
						...
						<div class="inventory col" v-if="!productData.IsNonInventory"><template v-if="showInventory">Qty </template>in stock</div>
						<div v-if="matrixDisplayPrice" class="matrix-price col">Price</div>
					...
					<div v-for="p in matrixProductsMatchingFilter" class="row">
						...
						<div :style="{ visibility: p.CanBeAddedToCart ? 'visible' : 'hidden' }" class="btn btn-secondary col col-add-cart" @click="addToCart(p, $event)" title="Add to cart"><i class="fa fa-shopping-cart"></i></div>
					</div>
				</div>
...
<div v-if="productData.Barcode"><br /><t>GlobalBarcode</t>: {{productData.Barcode}}</div>
				<div v-if="productData.CodeSupplier"><br />Supplier Code: {{productData.CodeSupplier}}</div>		
			</div>
		</div>
